...
export default class FileController {
  ...
  constructor(currentUri?: vscode.Uri) {
    ...
    const config = vscode.workspace.getConfiguration("rmg", this.currentUri);
  ...
  private async generateComponentTemplate(
    componentName: string,
    {
      defaultImports = [],
      exportType,
    }: { defaultImports: string[]; exportType: NewFileSettings["exportType"] }
  ) {
    let result = "";
    switch (exportType) {
      case "all":
  ...
  private getRootPathFromWorkspace(currentWorkspace?: vscode.WorkspaceFolder) {
